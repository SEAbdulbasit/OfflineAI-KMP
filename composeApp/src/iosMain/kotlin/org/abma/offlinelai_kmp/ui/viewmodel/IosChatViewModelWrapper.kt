...
// Wrapper for General Chat (no tools, no system prompt)
class IosGeneralChatWrapper {
    ...
    fun loadModel(modelPath: String) = viewModel.loadModel(modelPath, ModelConfig())
    fun sendMessage(text: String) { viewModel.updateInput(text); viewModel.sendMessage() }
    fun updateInput(text: String) = viewModel.updateInput(text)
    fun clearChat() = viewModel.clearChat()
    fun dismissError() = viewModel.dismissError()
    fun removeModel(path: String) = viewModel.removeLoadedModel(path)
    fun refreshModels() = viewModel.refreshLoadedModels()
    ...
    private fun ChatMessage.toIosMessage() = IosChatMessage(
        ...
        timestamp = timestamp, isStreaming = isStreaming, isError = isError
    )

    private fun LoadedModel.toIosModel() = IosLoadedModel(name = name, path = path, loadedAt = loadedAt)
}

// Wrapper for Actions (with tools and system prompt)
...
// Backward compatibility - delegates to IosActionsWrapper (full functionality)
